 ...
 */
class ThemeHouse_NewestPostFirs_Extend_XenForo_Model_Post extends XFCP_ThemeHouse_NewestPostFirs_Extend_XenForo_Model_Post
{
        ...
        if ($threadModel->getThreadDisplayOrderForThread($thread) == 'newest_first') {
            ...
            return $this->fetchAllKeyed($this->_getDb()
                ->limit('
    				...
    				AND (' . $stateLimit . ')
    				ORDER BY 
    				' . (XenForo_Application::get('options')->th_newestPostFirst_blogStyle ? 'FIELD(post.position, 0) DESC,' : '') . '
