...
class ThemeHouse_NewestPostFirs_Listener_TemplateHook extends ThemeHouse_Listener_TemplateHook
{
    ...
    {
        $templateHook = new ThemeHouse_NewestPostFirs_Listener_TemplateHook($hookName, $contents, $hookParams, $template);
    ...
    protected function _accountPreferencesOptions()
    {
        ...
        if ($xenOptions->th_newestPostFirst_userChoice) {
            $this->_appendTemplate('th_account_preferences_newestpostfirst');
        }
    }
    ...
    protected function _threadViewQrAfter()
    {
        ...
        if ($threadModel->getThreadDisplayOrderForThread($viewParams['thread']) == 'newest_first') {
            $this->_template->addRequiredExternal('js', 'js/themehouse/newestpostfirst/discussion.js');
        }
    }
}